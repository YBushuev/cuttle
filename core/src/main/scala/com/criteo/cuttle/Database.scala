...
private[cuttle] object Database {
    ...
    val schemaEvolutions = List(
      ...
      CREATE TABLE IF NOT EXISTS executions (
        id          CHAR(36) NOT NULL,
        job         VARCHAR(900) NOT NULL,
        start_time  TIMESTAMP NOT NULL,
        end_time    TIMESTAMP NOT NULL,
        context_id  VARCHAR(900) NOT NULL,
      ...
      CREATE TABLE IF NOT EXISTS paused_jobs (
        id          VARCHAR(900) NOT NULL,
         "user" VARCHAR(256) NOT NULL DEFAULT 'not defined user',
          "date" TIMESTAMP NOT NULL DEFAULT '1991-11-01 15:42:00' :: TIMESTAMP,
        PRIMARY KEY (id)
      );

      CREATE TABLE IF NOT EXISTS executions_streams (
    ...
    private def lockedTransactor(xa: HikariTransactor[IO]): HikariTransactor[IO] = {
      ...
      """.update.run
            locks <- sql"""
          SELECT locked_by, locked_at FROM locks WHERE current_timestamp - locked_at < '5 MINUTES' :: INTERVAL ;
        """.query[(String, Instant)].to[List]
            _ <- if (locks.isEmpty) {
                sql"""
            DELETE FROM locks;
            INSERT INTO locks VALUES (${guid}, current_timestamp);
          """.update.run
            } else {
...
private[cuttle] trait Queries {
    ...
    private[cuttle] def pauseJobQuery[S <: Scheduling](pausedJob: PausedJob) =
        sql"""
    ...
    def jobStatsForLastThirtyDays(jobId: String): ConnectionIO[List[ExecutionStat]] =
        sql"""
         select
             ...
             success
         from executions
         where job=$jobId and  current_timestamp - end_time < '30 DAYS' :: INTERVAL
